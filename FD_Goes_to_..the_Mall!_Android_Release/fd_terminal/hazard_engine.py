...
class HazardEngine:
    ...
    def _spawn_entities_for_hazard(self, hazard_id: str):
        ...
        if objs is None:
            room['objects'] = objs = []
        # Normalize existing names to avoid duplicates
        try:
            norm = self.game_logic._norm
        except Exception:
            norm = lambda s: str(s).strip().lower().replace('_', ' ')
        existing = {norm(o['name'] if isinstance(o, dict) else str(o)) for o in objs}

        items_master = self.game_logic.resource_manager.get_data('items', {})
        spawned = {}
        for key in entity_keys:
            # Normalize key to string for use as a dict key
            if isinstance(key, dict):
                key_str = key.get('id') or key.get('name') or str(key)
            else:
                key_str = str(key)
            display = self._choose_display_name_for_entity(key, hazard_def, items_master)
            if norm(display) not in existing:
                # Append as a simple string object entry (keeps current room schema)
                objs.append(display)
                existing.add(norm(display))
            spawned[key_str] = display

        hazard_inst['spawned_entities'] = spawned
        self.logger.info(f"Spawned hazard entities for '{hazard_type}' in '{room_name}': {list(spawned.values())}")
    ...
    def process_turn(self) -> dict:
        ...
        if self.game_logic and self.game_logic.death_ai:
            ...
            self.logger.debug(f"DeathAI returned messages: {ai_messages}")
            messages.extend(ai_messages)
        else:
            self.logger.debug("No DeathAI present or game_logic not set.")

        # --- Hazard Progression Logic ---
        for hazard_id, hazard in self.active_hazards.items():
            ...
            state_data = hazard.get('master_data', {}).get('states', {}).get(current_state_key, {})
            self.logger.debug(f"Hazard state: {current_state_key}, state_data: {state_data}")
    ...
    def _build_popup_consequence(self, hazard_id: str, new_state: str, popup_title: str, popup_message: str,
        ...
        """Construct a single show_popup event with optional deferred QTE/state change."""
        self.logger.debug(f"[_build_popup_consequence] Building popup consequence for hazard_id: {hazard_id}, new_state: {new_state}")
        popup_evt = {
            # Compatibility for both handlers
        ...
        # Only attach deferred actions if not already in terminal state
        hazard = self.active_hazards.get(hazard_id)
        if hazard and hazard.get('state') == "safe_exit_ending":
            self.logger.info(f"Popup for hazard '{hazard_id}' in terminal state; not attaching deferred actions.")
            return popup_evt

        # --- GUARD: Prevent repeated transitions in QTE chains ---
        # Track visited states for this hazard during the current QTE chain
        if hazard is not None:
            if "_visited_states" not in hazard:
                hazard["_visited_states"] = set()
            # If this state has already been visited in this QTE chain, do not attach further transitions
        ...
        # If there is a QTE on entry, defer it to popup dismiss
        if qte_entry:
            ...
            self.logger.debug(f"[_build_popup_consequence] Deferring QTE '{qte_entry.get('qte_type')}' to popup close for {hazard_id}:{new_state}")

        # If paused without QTE but with next_state, defer state change to popup dismiss
        if pause and next_state and not qte_entry:
            popup_evt["on_close_set_hazard_state"] = {
                "hazard_id": hazard_id,
                "target_state": next_state
            }
            self.logger.debug(f"[_build_popup_consequence] Deferring next_state -> '{next_state}' to popup close for {hazard_id}:{new_state}")

        self.logger.debug(f"[_build_popup_consequence] Built popup event: {popup_evt}")
        return popup_evt
    ...
    def set_hazard_state(self, hazard_id: str, new_state: str, suppress_entry_effects: bool = False, messages=None) -> dict:
        ...
        Now correctly processes special actions and rewards on state entry.
        Enhanced: QTE warnings are shown prior to QTE, with QTE chain logic.
        Improved: Tracks evaded hazards when reaching safe states.
        Only the first consequence (popup, QTE, or hazard state change) is returned per turn.
        """
        self.logger.debug(f"[set_hazard_state] Called for hazard_id='{hazard_id}', new_state='{new_state}'")
        hazard = self.active_hazards.get(hazard_id)
        if not hazard:
            self.logger.warning(f"[set_hazard_state] Unknown hazard id '{hazard_id}'")
            return {"consequences": []}

        current_state = hazard.get('state')
        if current_state == new_state:
            self.logger.info(f"[set_hazard_state] Ignoring repeated transition to state '{new_state}' for hazard '{hazard_id}'")
            return {"consequences": []}

        prev_state = current_state
        hazard['state'] = new_state
        self.logger.info(f"[set_hazard_state] Hazard '{hazard_id}' state changed from '{prev_state}' to '{new_state}'.")

        # --- PATCH: Handle triggers_hazard_on_state_change ---
        ...
        for trigger in triggers:
            t_hazard_type = trigger.get('hazard_type')
            t_location = trigger.get('location', location)
            t_state = trigger.get('initial_state')
            t_chance = trigger.get('chance', 1.0)
            if random.random() <= t_chance:
                # Find or create the hazard, but DO NOT set its state yet
                existing = None
                for h_id, h_inst in self.active_hazards.items():
                    if h_inst.get('type') == t_hazard_type and h_inst.get('location') == t_location:
                        existing = h_id
                        break
                if not existing:
                    existing = self._add_active_hazard(
                        hazard_type=t_hazard_type,
                        location=t_location,
                        initial_state_override=t_state
                    )
                # Enqueue a consequence to change its state (even if just created)
                if existing:
                ...
                if messages is not None:
                    messages.append(f"A new hazard '{t_hazard_type}' is triggered in {t_location} (state: {t_state}).")

        # Track evaded hazards when they reach safe states
        if new_state in ['inactive', 'resolved', 'evaded', 'neutralized', 'defused'] and self.game_logic:
            hazard_type = hazard.get('type', '')
            ...
            self.logger.info(f"[set_hazard_state] Added evaded hazard: {evaded_hazard['name']}")

        if suppress_entry_effects:
            return {"consequences": []}

        # Resolve state def and run entry actions
        sdef = self._resolve_state_def(hazard, new_state)
        self._apply_entry_actions(sdef, hazard_id)

        # Build only the first consequence for this turn
        popup_msg, popup_title, qte_entry, pause, next_state = self._extract_entry_metadata(sdef)

        # --- Terminal state handling: always show popup before transition ---
        if sdef.get('is_terminal_state') or sdef.get('instant_death_in_room'):
            is_death = bool(sdef.get('instant_death_in_room') or sdef.get('death_message'))
            popup_message = (
                sdef.get('death_message')
                if is_death else
                sdef.get('description') or "You survived!"
            )
            ...
            if is_death:
                # Enqueue game over after popup
                popup_event["on_close_emit_ui_events"] = [
                    {
                ...
                # Enqueue level complete after popup
                ...
                if self.game_logic:
                    ...
                    lcd = self.game_logic.get_level_completion_data() if hasattr(self.game_logic, 'get_level_completion_data') else {}
                    level_complete_event.update(lcd)
                popup_event["on_close_emit_ui_events"] = [level_complete_event]
            return {"consequences": [popup_event] + triggered_consequences}

        if popup_msg:
            consequence = self._build_popup_consequence(
                hazard_id, new_state, popup_title, popup_msg,
                qte_entry, pause, next_state
            )
            return {"messages": [], "consequences": [consequence] + triggered_consequences}

        if qte_entry and not popup_msg:
            consequence = self._build_immediate_qte_consequence(hazard_id, qte_entry)
            return {"messages": [], "consequences": [consequence] + triggered_consequences}

        if next_state and not pause:
            consequence = self._build_auto_advance_consequence(hazard_id, next_state)
            return {"messages": [], "consequences": [consequence] + triggered_consequences}

        return {
            "messages": [],
            "consequences": triggered_consequences
        }
    ...
    def _process_state_entry_rewards(self, sdef):
        ...
        for item_id in items:
            # Ensure inventory is a dictionary, not a list
            if 'inventory' not in self.game_logic.player:
                self.game_logic.player['inventory'] = {}
            elif isinstance(self.game_logic.player['inventory'], list):
                # Convert list inventory to dict (shouldn't happen but safety check)
                old_inventory = self.game_logic.player['inventory']
                self.game_logic.player['inventory'] = {}
                self.logger.warning(f"Converted player inventory from list {old_inventory} to dict")
            
            # Add the item
            item_data = self.game_logic.resource_manager.get_data('items', {}).get(item_id, {"name": item_id})
