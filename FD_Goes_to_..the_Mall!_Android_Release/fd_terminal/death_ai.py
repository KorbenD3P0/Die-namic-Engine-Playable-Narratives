...
class DeathAI:
    ...
    def _get_level_hallucinations(self, level: int, room: str) -> list:
        ...
        # Level 1: Movie Theater
        if level == 1:
            cineplex_hallucinations = [
                "The flickering lights seem to spell out a warning before returning to normal.",
                "You hear the distant sound of a film projector jamming, but the movie hasn't started yet.",
                "A popcorn machine sputters and hisses, but no one is near it.",
                "You catch a glimpse of someone in a theater seat, but when you look again, it's empty.",
                "The smell of burnt popcorn suddenly fills the air, sharp and acrid.",
                "A marquee letter rattles violently, then stops as if nothing happened.",
                "You hear a soda spill and a slip, but see no one fall.",
                "The reflection in the lobby window shows a fire in the projection booth, but when you blink, it's gone.",
                "A trailer on the overhead TV glitches, showing a flash of fire and a falling sign."
            ]
            return base_hallucinations + cineplex_hallucinations

        # Level 2: Hospital - Medical/clinical horror
        if level == 2:
    ...
    def _apply_level_specific_fear_effects(self, description: str, room_name: str, level: int) -> str:
        ...
        if level == 1:  # Movie Theater
            additions = {
                "Cineplex Lobby": " The neon lights seem to flicker in time with your heartbeat.",
                "Concessions": " The popcorn machine hisses, and you swear you see flames for a moment.",
                "Projection Corridor": " The hum of the projectors is almost hypnotic, but something feels off.",
                "Hallway to Theater 7": " The sticky floor seems to tug at your shoes, as if trying to hold you back.",
                "Theater 7": " The air feels heavy, and the shadows in the corners seem to move.",
                "Emergency Exit Corridor": " The exit sign flickers, casting strange shadows on the walls.",
                "Cineplex Parking Lot": " Sirens echo in the distance, and the neon reflections seem to ripple in the puddles."
            }
            return description + additions.get(room_name, " The atmosphere is thick with anticipation and dread.")
    ...
    def manifest_deaths_presence(self, location: str, intensity: float = None):
        ...
        active_hazards = self.game_logic.hazard_engine.get_active_hazards_for_room(location)
        deaths_breath_id = None
        for hazard_id in active_hazards:
            if "deaths_breath" in hazard_id:
                deaths_breath_id = hazard_id
                break

        # If intensity not specified, calculate from player fear
        if intensity is None:
            intensity = min(1.0, self.game_logic.player.get('fear', 0) * 1.5)

        # If already exists, possibly escalate based on intensity
        if deaths_breath_id:
            curr_state = self.game_logic.hazard_engine.get_hazard_state(deaths_breath_id, location)
            ...
            if random.random() < escalate_chance and curr_idx < len(states) - 1:
                ...
                self.game_logic.hazard_engine.set_hazard_state(deaths_breath_id, target_state)
                return True

        # Otherwise, spawn new instance if intensity is high enough
        elif intensity > 0.3:  # Only manifest at moderate fear levels
    ...
    def _escalate_immediate_threat(self, hazard_engine, params):
        ...
        # --- PATCH: Use level-appropriate threats ---
        current_level = self.game_logic.player.get('current_level', 1)
        if current_level == 1:  # Movie Theater
            immediate_threats = ['falling_marquee_letters', 'popcorn_oil_flareup', 'soda_spill_slip', 'overheating_projector_chain']
        elif current_level == 2:  # Hospital
            immediate_threats = ['electrical_surge', 'gas_explosion', 'faulty_wiring']
        else:
            immediate_threats = ['escalating_danger']
    ...
    def _contaminate_hiding_spot_enhanced(self, location: str, strategy: dict) -> bool:
        """Enhanced hiding spot contamination with level-appropriate hazards"""
        ...
        if 'overused_hiding_spot_' in reason:
            ...
            try:
                ...
                # --- LEVEL-APPROPRIATE HAZARDS ---
                current_level = self.game_logic.player.get('current_level', 1)
                if current_level == 1:
                    hazard_types = ['popcorn_oil_flareup', 'soda_spill_slip', 'sparking_neon_transformer']
                elif current_level == 2:
                    hazard_types = ['gas_leak', 'electrical_fault', 'structural_weakness']
                else:
                    hazard_types = ['escalating_danger']
    ...
    def _spawn_hazard_in_safe_zone(self, location: str, strategy: dict) -> bool:
        """Spawn hazards in locations where player feels safe, using level-appropriate hazards."""
        current_level = self.game_logic.player.get('current_level', 1)
        if current_level == 1:
            suitable_hazards = [
                'popcorn_oil_flareup', 'soda_spill_slip', 'sparking_neon_transformer',
                'falling_marquee_letters', 'overheating_projector_chain'
            ]
        elif current_level == 2:
            suitable_hazards = [
                'faulty_wiring', 'gas_leak', 'electrical_surge'
            ]
        else:
            suitable_hazards = ['escalating_danger']
    ...
    def _spawn_targeted_hazard(self, location: str, strategy: dict) -> bool:
        ...
        current_level = self.game_logic.player.get('current_level', 1)
        # Level-specific hazard selection
        if current_level == 1:
            if threat_score >= 15.0:
                hazard_type = "overheating_projector_chain"
            elif threat_score >= 10.0:
                hazard_type = "popcorn_oil_flareup"
            else:
                hazard_type = "soda_spill_slip"
        elif current_level == 2:
            if threat_score >= 15.0:
    ...
    def get_forced_hazard_activations(self, level_id, current_level_rooms):
        ...
        # Example logic: Always spawn at least one movie theater hazard in a utility room
        if current_level == 1:
            for room_name, room_data in current_level_rooms.items():
                if "Concessions" in room_name or "Projection" in room_name:
                    activation = {
                        "hazard_type": "popcorn_oil_flareup",
                        "location": room_name,
    ...
    def _apply_high_fear_effects(self, description, room_name):
        ...
        fear_modifications = {
            # Level 1: Cineplex
            "Cineplex Lobby": " The neon lights flicker violently, casting twisted shadows that seem to reach for you.",
            "Concessions": " The popcorn machine sputters and hisses, and you catch a glimpse of something burning inside.",
            "Projection Corridor": " The hum of the projectors is deafening, and you feel watched from the darkness above.",
            "Hallway to Theater 7": " The sticky floor seems to grip your shoes, as if trying to hold you in place.",
            "Theater 7": " The screen flickers with images you don't remember, and the seats creak as if someone unseen is shifting.",
            "Emergency Exit Corridor": " The exit sign buzzes and the air is thick with dread.",
            "Cineplex Parking Lot": " The distant sirens grow louder, and you see fleeting shapes in the reflections of puddles.",
            # Level 2/3/4 and generic
    ...
    def _apply_medium_fear_effects(self, description, room_name):
        ...
        fear_modifications = {
            # Level 1: Cineplex
            "Cineplex Lobby": " The lights seem dimmer, and you feel a chill as you enter.",
            "Concessions": " The smell of burnt popcorn lingers, making you uneasy.",
            "Projection Corridor": " The projectors' hum is louder than usual, echoing down the hall.",
            "Hallway to Theater 7": " You hear faint footsteps behind you, but when you turn, no one is there.",
            "Theater 7": " The darkness between the seats feels deeper than before.",
            "Emergency Exit Corridor": " The exit sign flickers, and the corridor feels longer than you remember.",
            "Cineplex Parking Lot": " The wind rattles loose trash, and you feel exposed under the flickering lights.",
            # Level 2/3/4 and generic
    ...
    def _handle_mri_control_interaction(self, item_data, rule, messages):
        ...
        mri_hazard_id = None
        for hid, h_inst in self.game_logic.hazard_engine.active_hazards.items():
        ...
        mri_hazard = self.game_logic.hazard_engine.active_hazards[mri_hazard_id]
        ...
        if current_state in allowed_deactivation_states:
            # Deactivate MRI
            self.game_logic.hazard_engine.set_hazard_state(mri_hazard_id, "safely_powered_down", messages)
