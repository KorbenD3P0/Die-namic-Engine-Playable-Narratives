...
class GameLogic:
    ...
    def start_new_game(self, character_class="Journalist", start_level=1):
        ...
        self.player = {
            ...
            "qte_active": False,
            "qte_context": {},
            "evaded_hazards": [],  # NEW: Initialize evaded hazards list
        }
        ...
        ui_events = []
        # Canonical fix: Always display first_entry_text if present, even on game start
    ...
    def _handle_qte_resolution(self, qte_result: dict) -> dict:
        """Handle QTE completion with proper consequence processing"""
        self.player['qte_active'] = False

        # Destroy QTE popup first
        self.add_ui_event({"event_type": "destroy_qte_popup", "priority": 1000})

        # Apply damage
        if not qte_result['success'] and qte_result.get('hp_damage', 0) > 0:
            self.player['hp'] = max(0, self.player['hp'] - qte_result['hp_damage'])
            # --- NEW: Trigger damage effect ---
            self.add_ui_event({"event_type": "player_damage_effect"})
            if qte_result.get('is_fatal', False) or self.player['hp'] <= 0:
                self.is_game_over = True
                # Prefer canonical hazard/cause if available
                death_reason = qte_result.get('death_reason') or self.player.get('death_reason') or "QTE failure"
                self.player['death_reason'] = death_reason
                self.logger.info("_handle_qte_resolution: Player died from QTE failure")
                self.add_ui_event({
        ...
        # Handle next hazard state - show success message first, then continue sequence
        hazard_id = qte_result.get('qte_source_hazard_id')
        if hazard_id and self.hazard_engine:
            next_state = (qte_result['next_state_success'] if qte_result['success'] 
                         else qte_result['next_state_failure'])
            if next_state:
                # Show QTE result popup that triggers next state when dismissed
                self.add_ui_event({
                    "event_type": "show_popup",
                    "priority": 50,
                    "title": "QTE Result", 
                ...
                # --- NEW: Handle pending move after QTE ---
                pending_move = self.player.pop('pending_move', None)
                if pending_move:
                    self.logger.info(f"QTE resolved, processing pending move: {pending_move}")
                    move_result = self._command_move(pending_move)
                    # --- NEW: Trigger low health effect if HP is low ---
                    if self.player['hp'] > 0 and self.player['hp'] <= 20:
        ...
        # No hazard state change needed, just show message
        self.add_ui_event({
            "event_type": "show_popup",
            "priority": 50,
            "title": "QTE Result",
    ...
    def _make_first_entry_popup_event(self, room_id: str, text: str) -> dict:
        ...
        # Always create a popup event if text is provided
        if text:
            return {
                "event_type": "show_popup",
            ...
            # Return None if no text is provided
            return None
    ...
    def _get_rich_room_description(self, room_id: str) -> str:
        ...
        object_names = [o['name'] for o in visible_entities['objects']]
    ...
    def check_game_state_transitions(self):
        ...
        if self.is_game_over:
        ...
        elif self.check_level_exit_available():
            # All requirements found, show popup (do NOT transition level)
        ...
        elif self.check_level_completion():
            # FIX: Get proper level completion data with current player stats
            ...
            # CRITICAL FIX: Use current player stats, not cached level_data
            self.add_ui_event({
    ...
    def get_death_narrative(self) -> str:
        """
        Builds a rich death narrative for the end screen, including player stats,
        cause of death, evidence collected, hazards encountered, and a narrative
        from their intro disaster or a special ending if set.
        """
        # 1. Use special narrative if set (e.g., secret ending or custom death)
        narrative = ""
        custom_narrative = self.player.get('death_narrative')
        if custom_narrative:
            narrative += f"{custom_narrative}\n"

        # 2. Always append stats and intro disaster for closure
        hazards_encountered = len(self.player.get('evaded_hazards', []))
        evidence_count = len(self.player.get('inventory', []))
        turns_survived = self.player.get('actions_taken', 0)
        intro_disaster = self.player.get('intro_disaster', {})

        narrative += f"\nMajor Stats:\n"
        narrative += f"- Hazards encountered: {hazards_encountered}\n"
        narrative += f"- Evidence collected: {evidence_count}\n"
        narrative += f"- Turns survived: {turns_survived}\n"
        narrative += f"\nYour story began with {intro_disaster.get('event_description', '')}\n"

        # 3. If not a special ending, append the standard disaster death narrative
        if not custom_narrative:
            disaster_key = intro_disaster.get('event_description', '')
            disasters = self.resource_manager.get_data('disasters', {})
            disaster_data = disasters.get(disaster_key, {})
            death_narrative_text = disaster_data.get('death_narrative', '')
            if death_narrative_text:
                narrative += f"\n{death_narrative_text}\n"
    ...
    def _force_try_exit(self, target_name: str, tool_key, bonus, room, exits) -> Optional[dict]:
        ...
        try:
            ...
            if matched:
                ...
                if not room.get('forceable') and not dest_data.get('forceable'):
                    msg = "There's nothing to force that way."
                    self.logger.info(f"_force_try_exit: {msg}")
                    return self._build_response(message=msg, turn_taken=False)
                ...
                # Start a generic force-door QTE
                prompt = f"The door resists! {'Use your tool! ' if tool_key else ''}Mash to force it!"
                ctx = {
                    "ui_mode": "in-screen",
                    "ui_prompt_message": prompt,
                    "target_mash_count": tgt_mash,
                    "duration": 4.0,
                    "success_message": f"You wrench the door to {dest_room_id} open just enough to slip through!",
