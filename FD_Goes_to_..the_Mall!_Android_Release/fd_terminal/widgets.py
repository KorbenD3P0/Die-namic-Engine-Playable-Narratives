...
class OutputPanelWidget(BoxLayout):
    ...
    def __init__(self, **kwargs):
        ...
        self.output_label = Label(
            ...
            font_size=sp(15),  # Changed from dp(15) to sp(15)
            size_hint_y=None,
            valign='top',
            halign='left',
            padding=(dp(8), dp(8))
        )
        self.output_label.bind(texture_size=lambda i, v: setattr(i, 'height', v[1]))
        # Wrap to ScrollView width
        self.output_scroll_view = ScrollView(size_hint_y=1)
        self.output_scroll_view.add_widget(self.output_label)
        self.output_scroll_view.bind(
            width=lambda i, w: setattr(self.output_label, 'text_size', (w - dp(16), None))
        )
    ...
    def append_text(self, text_to_append, clear_previous=False):
        ...
        # Ensure color tags are properly closed if text is truncated
    ...
    def _ensure_color_tags_closed(self, text):
        ...
        # If there are unclosed color tags, close them
        if open_tags > 0:
...
class MapDisplayWidget(BoxLayout):
    ...
    def __init__(self, **kwargs):
        ...
        # Optional header (can be removed if space is tight)
        ...
        # CRITICAL FIX: Wrap map_label in a ScrollView with constrained height
        map_scroll = ScrollView(
            size_hint_y=1,  # Fill remaining space in parent
            do_scroll_x=False,
            do_scroll_y=True,
            bar_width=dp(8)
        )
        ...
        self.map_label = Label(
            text="",
            font_name='RobotoMono',
            font_size=dp(12),  # Slightly smaller for more compact display
            markup=True,
            halign='center',
            valign='top',
            size_hint_y=None,
        ...
        # Height grows with content, but scroll view clips it
        self.map_label.bind(
            texture_size=lambda *args: setattr(self.map_label, 'height', self.map_label.texture_size[1])
        )
        # Enable text wrapping (though map should already be pre-formatted)
        self.map_label.bind(
            width=lambda *args: setattr(self.map_label, 'text_size', (self.map_label.width - dp(8), None))
        )
...
class ContextualActionsWidget(ScrollView):
    ...
    def __init__(self, **kwargs):
        ...
        self.grid = GridLayout(cols=6, spacing=dp(3), size_hint_y=None)
...
class QTEPopup(Popup):
    ...
    def __init__(self, prompt, duration, input_type, submit_callback, qte_context=None, **kwargs):
        ...
        # Prompt label (wrapped to popup width on resize for phone screens)
        self.prompt_label = Label(text=f"[b]{prompt}[/b]", markup=True, halign='center')
        def _wrap_prompt(*_):
            pad = dp(20)
            # Ensure the label wraps to the popup's current width and grows to fit texture
            self.prompt_label.text_size = (max(0, self.width - pad), None)
            self.prompt_label.size_hint_y = None
            # Ensure texture is up-to-date before reading texture_size
            self.prompt_label.height = self.prompt_label.texture_size[1] + dp(8)
        # Update when popup size changes and initialize immediately
        self.bind(size=_wrap_prompt)
        _wrap_prompt()
    ...
    def _qte_ui_mash(self, layout):
        ...
        self.mash_counter = Label(
            text=f"Presses: {self.mash_count}",
            font_size='24sp',
            size_hint_y=None,
            height='40dp'
        )
        ...
        btn.bind(on_release=lambda *_: self.submit_callback({'event': 'mash_press', 'count': self.mash_count + 1}))
        layout.add_widget(btn)
        self.text_input = None
        eff = self.qte_context.get('effective_target_mash_count')
        if isinstance(eff, (int, float)):
            self.target_label = Label(
                text=f"Target: {int(eff)}",
                font_size='18sp',
                size_hint_y=None,
                height='30dp'
            )
    ...
    def _qte_ui_tap(self, layout):
        ...
        btn = Button(text="Tap!", font_size='32sp', size_hint_y=None, height='60dp')
        btn.bind(on_release=lambda *_: self.submit_callback({'event': 'tap', 'count': self.tap_count + 1}))
        layout.add_widget(btn)
        self.text_input = None
    ...
    def dismiss(self, *largs, **kwargs):
        ...
        class QTEButtonWidget(BoxLayout):
            ...
            def _build_ui(self):
                if self.input_type in ('mash', 'tap', 'tap_count', 'precision_tap_count'):
                    ...
                    btn = Button(text=label, font_size=sp(32), size_hint=(1, 0.8))
                    # Ensure the button’s text wraps/shortens if needed
                    btn.shorten = True
                    btn.bind(size=lambda inst, *_: setattr(inst, 'text_size', (inst.width - dp(12), None)))
                    btn.bind(on_release=self._on_tap_or_mash)
                    self.add_widget(btn)

                elif self.input_type in ('hold', 'hold_release', 'hold_and_release'):
                    btn = Button(text="Hold!", font_size=sp(32), size_hint=(1, 0.8))
                    btn.shorten = True
                    btn.bind(size=lambda inst, *_: setattr(inst, 'text_size', (inst.width - dp(12), None)))
                    btn.bind(on_touch_down=self._on_hold_down)
                    btn.bind(on_touch_up=self._on_hold_up)
                    ...
                    for i, label in enumerate(["Left", "Right"]):
                        btn = Button(text=label, font_size=sp(32), background_color=self._alt_color(i))
                        btn.shorten = True
                        btn.bind(size=lambda inst, *_: setattr(inst, 'text_size', (inst.width - dp(12), None)))
                    ...
                    btn = Button(text="Action!", font_size=sp(32), size_hint=(1, 0.8))
                    btn.shorten = True
                    btn.bind(size=lambda inst, *_: setattr(inst, 'text_size', (inst.width - dp(12), None)))
        ...
        class ContextDockWidget(BoxLayout):
            ...
            def __init__(self, **kwargs):
                ...
                # CRITICAL FIX: Wrap context_grid in a ScrollView with fixed height
